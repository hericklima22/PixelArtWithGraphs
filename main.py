...
TAXA_COR = 10                    # muda a frequencia com que cada cor é alterada, quanto maior, mais cores aparecerão (melhor efeito entre 16 e 100)
vertices = []
...
class Vortex:
  ...
  def vortex(self, display, color=None):
    ...
    pygame.display.update()
    # t = Thread(target=pygame.display.update)
    # threads.append(t)
    # t.start()
...
def escolhe_cor(cor, taxa_hue=0.05, taxa_lightness=0.1, limite_lightness=(0.2, 0.8)):
  ...
  # Converter RGB para HLS
  h, l, s = colorsys.rgb_to_hls(r, g, b)

  # Alterar a matiz (hue) suavemente
  h = (h + random.uniform(-taxa_hue, taxa_hue)) % 1.0

  # Alterar a luminosidade (lightness), respeitando os limites
  l = max(limite_lightness[0], min(limite_lightness[1], l + random.uniform(-taxa_lightness, taxa_lightness)))

  # Converter de volta para RGB
  r, g, b = colorsys.hls_to_rgb(h, l, s)

  # Retornar os valores RGB normalizados para [0, 255]
